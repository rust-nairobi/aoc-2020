...
fn fix_expense_report(report: &[i32]) -> i32 {
    for (idx1, i) in report.iter().enumerate() {
        for (idx2, j) in report.iter().enumerate() {
            if (idx1 != idx2) && (i + j == 2020) {
                return i * j;
            }
        }
    }
...
fn test_solution() {
    assert_eq!(
        fix_expense_report(&vec![1721, 979, 366, 299, 675, 1456]),
        514579
    )
}
